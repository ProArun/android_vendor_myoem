...
#define LOG_TAG "VtsHwCalculatorServiceTest"

// ─────────────────────────────────────────────────────────────────────────────
// What this file tests
//
//   IHwCalculatorService — a vendor C++ binder service that exposes:
//     add(a, b)       → a + b
//     subtract(a, b)  → a - b
//     multiply(a, b)  → a * b
...
// AIDL-generated NDK proxy for IHwCalculatorService.
// Generated from: services/hwcalculator/aidl/com/myoem/hwcalculator/IHwCalculatorService.aidl
...
//   1. kServiceName in services/hwcalculator/src/main.cpp
//   2. The label in services/hwcalculator/sepolicy/private/service_contexts
//
// If any of the three disagree, the service either won't register or won't
// be discoverable by clients.
// ─────────────────────────────────────────────────────────────────────────────
static constexpr const char* kServiceName = "com.myoem.hwcalculator.IHwCalculatorService";
...
// ─────────────────────────────────────────────────────────────────────────────
class HwCalculatorServiceTest : public ::testing::Test {
  public:
    ...
    static std::shared_ptr<IHwCalculatorService> sService;
    ...
    static void SetUpTestSuite() {
        ...
        if (binder.get() != nullptr) {
            ...
            sService = IHwCalculatorService::fromBinder(binder);
        }
        // If sService remains null, each individual test will call GTEST_SKIP().
    }

  protected:
    // Called before EACH test — skip cleanly if hwcalculatord is not running.
    ...
    void SetUp() override {
        if (sService == nullptr) {
            GTEST_SKIP() << "hwcalculatord not running — is it installed and started?";
        }
    }
};

// Static member definition — required by C++ (declaration is in the class above)
std::shared_ptr<IHwCalculatorService> HwCalculatorServiceTest::sService = nullptr;
...
TEST_F(HwCalculatorServiceTest, ServiceRegistered) {
...
TEST_F(HwCalculatorServiceTest, Add_TwoPositives) {
    int32_t result = 0;
...
TEST_F(HwCalculatorServiceTest, Add_TwoNegatives) {
    int32_t result = 0;
...
TEST_F(HwCalculatorServiceTest, Add_PositiveAndNegative) {
    int32_t result = 0;
...
TEST_F(HwCalculatorServiceTest, Add_BothZero) {
    int32_t result = 0;
...
TEST_F(HwCalculatorServiceTest, Add_WithZero) {
    int32_t result = 0;
...
TEST_F(HwCalculatorServiceTest, Add_MaxPlusZero) {
    int32_t result = 0;
...
TEST_F(HwCalculatorServiceTest, Add_InversesResultInZero) {
    int32_t result = 0;
...
TEST_F(HwCalculatorServiceTest, Subtract_PositiveResult) {
    int32_t result = 0;
...
TEST_F(HwCalculatorServiceTest, Subtract_NegativeResult) {
    int32_t result = 0;
...
TEST_F(HwCalculatorServiceTest, Subtract_SameNumbers) {
    int32_t result = 0;
...
TEST_F(HwCalculatorServiceTest, Subtract_FromZero) {
    int32_t result = 0;
...
TEST_F(HwCalculatorServiceTest, Subtract_TwoNegatives) {
    int32_t result = 0;
...
TEST_F(HwCalculatorServiceTest, Multiply_TwoPositives) {
    int32_t result = 0;
...
TEST_F(HwCalculatorServiceTest, Multiply_ByZero) {
    int32_t result = 0;
...
TEST_F(HwCalculatorServiceTest, Multiply_TwoNegatives) {
    int32_t result = 0;
...
TEST_F(HwCalculatorServiceTest, Multiply_MixedSign) {
    int32_t result = 0;
    ASSERT_BINDER_OK(sService->multiply(-3, 4, &result));
    EXPECT_EQ(result, -12);
}
...
TEST_F(HwCalculatorServiceTest, Multiply_ByOne) {
    int32_t result = 0;
...
TEST_F(HwCalculatorServiceTest, Divide_ExactDivision) {
    int32_t result = 0;
...
TEST_F(HwCalculatorServiceTest, Divide_IntegerTruncation) {
    int32_t result = 0;
...
TEST_F(HwCalculatorServiceTest, Divide_NegativeByPositive) {
    int32_t result = 0;
    ASSERT_BINDER_OK(sService->divide(-10, 2, &result));
    EXPECT_EQ(result, -5);
}
...
TEST_F(HwCalculatorServiceTest, Divide_NegativeByNegative) {
    int32_t result = 0;
    ASSERT_BINDER_OK(sService->divide(-10, -2, &result));
    EXPECT_EQ(result, 5);
}
...
TEST_F(HwCalculatorServiceTest, Divide_ByOne) {
    int32_t result = 0;
...
TEST_F(HwCalculatorServiceTest, Divide_ByMinusOne) {
    int32_t result = 0;
    ASSERT_BINDER_OK(sService->divide(42, -1, &result));
    EXPECT_EQ(result, -42);
}
...
TEST_F(HwCalculatorServiceTest, Divide_ZeroByNonZero) {
    int32_t result = 0;
...
TEST_F(HwCalculatorServiceTest, Divide_ByZero_ReturnsErrorStatus) {
    int32_t result = 0;
...
TEST_F(HwCalculatorServiceTest, Divide_ByZero_ErrorCode_IsOne) {
    int32_t result = 0;
    ndk::ScopedAStatus status = sService->divide(10, 0, &result);
...
TEST_F(HwCalculatorServiceTest, ErrorConstant_DivideByZero_IsOne) {
    ...
    EXPECT_EQ(IHwCalculatorService::ERROR_DIVIDE_BY_ZERO, 1);
}
...
TEST_F(HwCalculatorServiceTest, ConcurrentCalls_NoDataRace) {
...
TEST_F(HwCalculatorServiceTest, RepeatedCalls_ServiceStable) {
